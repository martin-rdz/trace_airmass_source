...
def plot_source_2d(f, parameter, dt_list, dsp, config, savepath):
    ...
    for it, dt in enumerate(dt_list):
        ...
        if dsp == 'abs':
            ...
            axes[it].xaxis.set_major_locator(matplotlib.ticker.MultipleLocator(2500))
    ...
    plt.suptitle("{} {}".format(dt.strftime("%Y%m%d"), parameter), 
...
def plot_geonames_2d(f, parameter, dt_list, dsp, config, savepath):
    ...
    assert geo_names == ast.literal_eval(f.variables[parameter].comment)
    ...
    axes[0].annotate('Time UTC', xy=(.5, .0),
                     ...
                     horizontalalignment='center', verticalalignment='bottom',
                     fontsize=14, fontweight='semibold')
    # axes[-1].annotate(dsp_text, xy=(.82, 0.85),
    #                 xycoords='figure fraction',
    #                 horizontalalignment='center', verticalalignment='bottom',
    #                 fontsize=12, fontweight='semibold')
...
def plot_source_height_profile(f, parameter, dt, it, config, savepath):
    time_list = f.variables["timestamp"][:]
    dt_list = [datetime.datetime.fromtimestamp(time) for time in time_list]
    height_list = f.variables["range"][:]
    no_occ = f.variables[parameter + '_no_below'][it, :]
...
def plot_filename(filename, config_file='config.toml'):
    ...
    print(f.variables['min_height_ag'])
    print(f.variables['min_height_ag'][:])
    ...
    for rh in config['height']['reception']:
            if rh != 'md':
                ...
                rh_string = rh
            plot_source_2d(f, 'occ_ens_below'+rh_string, dt_list, 'abs', config, savepath)
            plot_source_2d(f, 'occ_ens_below'+rh_string, dt_list, 'rel', config, savepath)
            plot_geonames_2d(f, 'region_ens_below'+rh_string, dt_list, 'abs', config, savepath)
            plot_geonames_2d(f, 'region_ens_below'+rh_string, dt_list, 'rel', config, savepath)

    for dt in dt_list:
        it = dt_list.index(dt)
        plot_source_height_profile(f, 'occ_belowmd', dt, it, config, savepath)
        plot_source_height_profile(f, 'occ_ens_belowmd', dt, it, config, savepath)
        plot_source_height_profile(f, 'occ_below2.0km', dt, it, config, savepath)
        plot_source_height_profile(f, 'occ_ens_below2.0km', dt, it, config, savepath)
...
for filename in filelist:
    ...
    plot_filename(filename, config_file=config)
